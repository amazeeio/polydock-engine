    ...
    public static function getColumns(): array
    {
        return [
            ...
            ExportColumn::make('status')
                ->state(fn (PolydockAppInstance $record): string => $record->getStatus()->toString())
                ->label('Registration Status'),
            ExportColumn::make('email')
                ->state(fn (PolydockAppInstance $record): mixed => UserRemoteRegistrationExporter::getValueFromData(
                    $record->data,
                    'user-email',
                ))
                ->label('Email Address'),
