...
trait PostCreateAppInstanceTrait
{
    ...
    public function postCreateAppInstance(PolydockAppInstanceInterface $appInstance): PolydockAppInstanceInterface
    {
        ...
        try {
            ...
            $chainlitAuthSecret = $appInstance->getKeyValue('chainlit-auth-secret');
            if (empty($chainlitAuthSecret)) {
                $chainlitAuthSecret = $this->generateSecurePassword(64);
                $appInstance->storeKeyValue('chainlit-auth-secret', $chainlitAuthSecret);
            }
            $this->postCreateLagoonOps?->addOrUpdateLagoonProjectVariable($appInstance, 'CHAINLIT_AUTH_SECRET', $chainlitAuthSecret, 'GLOBAL');

            $mcpAuthSecret = $appInstance->getKeyValue('mcp-auth-secret');
            if (empty($mcpAuthSecret)) {
                $mcpAuthSecret = $this->generateSecurePassword(64);
                $appInstance->storeKeyValue('mcp-auth-secret', $mcpAuthSecret);
            }
            $this->postCreateLagoonOps?->addOrUpdateLagoonProjectVariable($appInstance, 'MCP_AUTH_SECRET', $mcpAuthSecret, 'GLOBAL');

            $oauthDrupalClientSecret = $appInstance->getKeyValue('oauth-drupal-client-secret');
            if (empty($oauthDrupalClientSecret)) {
                $oauthDrupalClientSecret = $this->generateSecurePassword(64);
                $appInstance->storeKeyValue('oauth-drupal-client-secret', $oauthDrupalClientSecret);
            }
            $this->postCreateLagoonOps?->addOrUpdateLagoonProjectVariable($appInstance, 'OAUTH_DRUPAL_CLIENT_SECRET', $oauthDrupalClientSecret, 'GLOBAL');
