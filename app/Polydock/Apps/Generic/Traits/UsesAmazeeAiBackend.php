...
trait UsesAmazeeAiBackend
{
    ...
    public function checkAmazeeAiBackendAuth(): bool
    {
        ...
        if (! is_array($response)) {
            $this->error('Amazee AI backend auth check returned empty or non-array response', $logContext);

            return false;
        }

        if (! isset($response['is_admin']) || ! $response['is_admin']) {
        ...
        if (! isset($response['is_active']) || ! $response['is_active']) {
