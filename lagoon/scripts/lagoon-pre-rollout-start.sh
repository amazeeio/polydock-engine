...
if [ ! -z "$POLYDOCK_SRE_SLACK_WEBHOOK_URL" ]; then
    ...
    curl -X POST -H 'Content-type: application/json' --data '{"text":":white_circle:  [$SERVICE_NAME.$LAGOON_GIT_SAFE_BRANCH.$LAGOON_PROJECT] Pre-rollout Started"}' $POLYDOCK_SRE_SLACK_WEBHOOK_URL
fi
